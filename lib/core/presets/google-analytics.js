...
module.exports = function (options) {
    ...
    function getStartOfNextWindow() {
        return moment.tz('America/Los_Angeles').endOf('day').add('1', 'minute').valueOf();
    ...
    const backoff = {
        type: 'exponential',
        shouldBackoff: e => {
            return isGaError(e, 403, ['userRateLimitExceeded', 'rateLimitExceeded', 'quotaExceeded']) ||
            ...
            // 50,000 requests per project per day – can be increased
            {
                ...
                onError: (throttling, e) => {
                    if (isGaError(e, 403, 'dailyLimitExceeded')) {
                        console.log('quota (google-anayltics): dailyLimitExceeded');
                        throttling.saturate();
                    }
                }
            },
