            ...
            try {
                if (this.backoff) {
                    if (!await this.backoff.waitIfNecessary()) {
                        return reject(new errors.BackoffLimitError(this.name));
                    }
                }

                for (const rule of relevantRules) {
                    if (!rule.isAvailable(scope, resources, queuedRequest)) {
                        queuedRequest = await rule.enqueue(scope, options, queuedRequest);
                        if (!queuedRequest || !queuedRequest.isValid) {
                            // Between the call of queuedRequest.process() and enqueueing the request again the abort timer may have fired!
                            return reject(new errors.OutOfQuotaError(this.name));
                        } else if (rule !== queuedRequest.previouslyQueuedForRule) {
                            queuedRequest.scopeBundle.moreAvailable();
                        }

                        return this.requestQuota(scope, resources, options, queuedRequest).then(resolve, reject);
                    }
                }
            } catch (e) {
                return reject(e);
