...
class Rule {
    constructor(options) {
        ...
        if (!_.isUndefined(options.name) && !_.isString(options.name)) {
            throw new Error(
                'Please pass either undefined or a string to options.name'
            );
        }

        if (
            !_.isUndefined(options.limit) &&
            (!_.isFinite(options.limit) || options.limit <= 0)
        ) {
            throw new Error('Please pass a positive integer to options.limit');
        }

        if (
            !_.isUndefined(options.window) &&
            (!_.isFinite(options.window) || options.window <= 0)
        ) {
        ...
        if (
            ...
            !_.isPlainObject(options.throttling)
        ) {
            throw new Error(
                'Please pass either a string or an object to options.throttling'
            );
        } else if (
            _.isPlainObject(options.throttling) &&
            !_.isString(options.throttling.type)
        ) {
            throw new Error(
                'Please pass the name of the throttling module to options.throttling.type'
            );
        }

        if (
            !_.isUndefined(options.queueing) &&
            !_.isString(options.queueing) &&
            !_.isPlainObject(options.queueing)
        ) {
            throw new Error(
                'Please pass either undefined, a string, or an object to options.queueing'
            );
        } else if (
            _.isPlainObject(options.queueing) &&
            !_.isString(options.queueing.type)
        ) {
            throw new Error(
                'Please pass the name of the queueing module to options.queueing.type'
            );
        }
        ...
        if (_.isUndefined(options.scope)) {
            ...
            _.forEach(options.scope, function(entry) {
                if (!_.isString(entry) || entry === '') {
                    throw new Error(
                        'Please pass only strings into the options.scope array.'
                    );
                }
            });
        } else {
            throw new Error(
                'Please pass either undefined, a string, or an array of strings to options.scope'
            );
        }

        if (!_.isUndefined(options.resource) && !_.isString(options.resource)) {
            throw new Error(
                'Please pass either undefined or a string to options.resource'
            );
        }
        ...
        this.ThrottlingFactory = loader.loadThrottlingFactory(
            options.throttling
        );
        if (options.queueing) {
    ...
    enqueue(scope, options, queuedRequest) {
        ...
        return new Promise((resolve, reject) => {
            queuedRequest = new QueuedRequest({
                rule: this,
                previouslyQueuedForRule: queuedRequest && queuedRequest.rule,
                scopeBundle,
                resolve,
                reject
            });

            if (_.isPlainObject(options) && _.isFinite(options.maxWait)) {
                setTimeout(() => {
                    queuedRequest.abort();
                }, options.maxWait + 2);
            }

            queuedRequest.scopeBundle.queueing[
                this === queuedRequest.previouslyQueuedForRule
                    ? 'addAgain'
                    : 'add'
            ](queuedRequest);
        });
     ...
     * @param {*} scope
     * @param {*} resources
     * @param {boolean} hasBeenQueuedBefore
     */
    isAvailable(scope, resources, hasBeenQueuedBefore) {
        const scopeBundle = this._getBundleForScope(scope);

        // if it wasn't queued before and there are items waiting in queue, return unavailable
        // this gives priority to queued items first
        if (
            scopeBundle.queueing &&
            !hasBeenQueuedBefore & (scopeBundle.queueing.getNumberWaiting() > 0)
        ) {
            return false;
        }
    ...
    _getBundleForScope(scope) {
        ...
        if (!bundles[scopeString]) {
            bundles[scopeString] = new ScopeBundle(
                this.ThrottlingFactory,
                this.QueueingFactory
            );
        }
    ...
    _getResourceAmount(resources) {
        ...
        if (!resourceName) {
            ...
            if (resourceNames.length === 1) {
                ...
                throw new Error(
                    'Please pass the resource parameter to your rules to allow requesting quota for mixed resource amounts'
                );
            }
        }
        ...
        if (!_.isFinite(amount)) {
            throw new Error(
                `Please pass a number to resources["${resourceName
                    .replace(/\\/g, '\\\\')
                    .replace(/\"/g, '\\"')}"]`
            );
        }
    ...
    _formatScope(scope) {
        ...
        for (const scopeName of this.options.scope) {
            if (!scope || _.isUndefined(scope[scopeName])) {
                throw new Error(
                    `Please pass a value for the "${scopeName}" scope with your quota request`
                );
            }
