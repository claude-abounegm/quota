...
class Rule {
    ...
    enqueue(scope, options, queuedRequest) {
        ...
        return new Promise((resolve, reject) => {
            ...
            queuedRequest.scopeBundle.queueing[this === queuedRequest.previouslyQueuedForRule ? 'addAgain' : 'add'](queuedRequest);
        });
    }
    ...
    isAvailable(scope, resources, hasBeenQueuedBefore) {
        ...
        if (scopeBundle.queueing && !hasBeenQueuedBefore & scopeBundle.queueing.getNumberWaiting() > 0) {
            return false;
        }
