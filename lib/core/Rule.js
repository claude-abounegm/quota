...
class Rule {
    constructor(options) {
        ...
        this.ThrottlingFactory = loader.loadThrottlingFactory(_.assign({
            limit: options.limit,
    ...
    _getBundleForScope(scope) {
        ...
        if (!this._throttling[scopeString]) {
            // const scopeBundle = new ScopeBundle(this.ThrottlingFactory, this.QueueingFactory);

            const scopeBundle = {
                moreAvailable: _.noop
            };

            if (this.QueueingFactory) {
                scopeBundle.queueing = this.QueueingFactory();
            ...
            scopeBundle.throttling = this.ThrottlingFactory(scopeBundle.moreAvailable);
